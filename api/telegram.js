...
async function handleUpdate(update, env) {
  ...
  if (cmd === "help" || cmd === "menu") {
    ...
    await reply(env, chatId, [
      ...
      "",
      "Mumu dari data/mumu.json, Putarin dari data/putarin.json."
    ].join("\n"), MENU_KEYBOARD);
...
function cleanTitle(t) {
  return String(t || "Video").replace(/^\u25b6\s*/, "").replace(/^\ud83c\udfac\s*/, "").replace(/\s*-\s*koleksidrpinguin.*/i, "").replace(/\s*\(Koleksi Dr\. Pinguin.*$/i, "").replace(/_/g, " ").replace(/\s+/g, " ").trim();
}
...
async function handleShare(env, chatId, opt) {
  ...
  if (!pool.length) {
    const msg = catTotal === 0
      ? ("Kategori " + catLabel + " kosong.")
      : ("Stok " + catLabel + " sesi ini habis (" + catTotal + " sudah dikasih 24 jam).");
...
function toItem(url) {
  ...
  if (low.includes("videy.co")) {
    ...
    try {
      ...
      id = (url.match(/\/(?:e|v|watch)\/([A-Za-z0-9_-]+)/i) || [])[1] || "";
