...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (msg && env.BOT_TOKEN) {
      try { await reply(env, msg.chat.id, "Error bot: " + String(e.message || e)); } catch (_) {}
    }
...
async function handleUpdate(update, env) {
  ...
  if (cmd === "help") {
    await reply(env, chatId, "Bot siap.\nKetik: minta\nAtau kirim link videy / vicek / indoav / userbokep");
...
async function readShareState(env) {
  const owner = env.GH_OWNER;
  const repo = await stateRepo(env);
  try {
...
function parseNamedLinks(text) {
  ...
  for (const rawLine of lines) {
    ...
    if (m) {
      ...
      const it = toItem(url);
      if (pending) it.title = cleanTitle(pending);
      items.push(it);
      pending = "";
