(function () {
  ...
  function blocked(v) {
    var u = embedOf(v);
    return /videy|putarin|puterin/i.test(u);
  }
  function fill() {
    ...
    var items = pick(mumu.filter(function (v) { return !blocked(v); }), 8);
  ...
  fetch("/data/mumu.json").then(function (r) { return r.json(); }).then(function (arr) {
    mumu = arr || [];
    fill();
